...
(() => {
    ...
    if (!PrecacheUiSupport || !PrecacheUiDom || !PrecacheUiFiles || !PrecacheUiLogs || !PrecacheUiSettings || !PrecacheUiStats) {
        throw new Error('[PrecacheUI] support scripts must load before app/index.js.');
    }
    const {
        createEmptyMetrics,
    ...
    function initNode() {
        ...
        precacher = req(path.join(precacherDir, 'cli', 'extract.js'));
        pretranslator = req(path.join(precacherDir, 'cli', 'translate.js'));
    }
