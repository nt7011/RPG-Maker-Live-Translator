...
(() => {
    ...
    function resolveRuntimeScriptOptions(loaderOptions = {}, settings = null) {
        return Object.assign({}, loaderOptions, {
            disableUiLauncher: loaderOptions.disableUiLauncher === true
                || !!(settings && settings.disableLiveTranslatorGui === true),
        });
    }
    ...
    function normalizeRuntimeManifest(manifest) {
        ...
        return {
            ...
            recommendedNwVersion: runtime.recommendedNwVersion || '',
    ...
    function detectNwVersion(logger, minNwVersion, recommendedNwVersion) {
        const recommendedVersion = recommendedNwVersion || minNwVersion;
        if (!minNwVersion && !recommendedVersion) return;
        try {
            ...
            if (minNwVersion && compareVersions(nwVersion, minNwVersion) < 0) {
                logger.warn(`[LiveTranslator][Compat] Detected NW.js version ${nwVersion}; below minimum ${minNwVersion}. Update NW.js before running the translator.`);
            } else if (recommendedVersion && compareVersions(nwVersion, recommendedVersion) < 0) {
                logger.warn(`[LiveTranslator][Compat] Detected NW.js version ${nwVersion}; below recommended ${recommendedVersion}. Update NW.js to avoid runtime and translation issues.`);
            } else {
    ...
    async function bootstrap() {
        ...
        try {
            ...
            let scriptPlan = createRuntimeScriptPlan(manifest, loaderOptions);
            ...
            detectNwVersion(logger, manifest.minNwVersion, manifest.recommendedNwVersion);
            ...
            scriptPlan = createRuntimeScriptPlan(
                manifest,
