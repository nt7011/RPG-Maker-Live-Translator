...
(() => {
    ...
    function createWindowRegistryHelpers(context = {}) {
        ...
        function isWindowRefreshActive(window, windowData) {
            return !!((window && Number(window._trWindowRefreshDepth) > 0)
                || (window && Number(window._trTranslationRefreshDepth) > 0)
                || (windowData && Number(windowData._trWindowRefreshDepth) > 0)
        ...
        function findUndrainedRefreshToken(windowData, currentRevision) {
            if (!windowData || !windowData.renderQueue || typeof windowData.renderQueue.forEach !== 'function') return 0;
            let token = 0;
            try {
                windowData.renderQueue.forEach((queued) => {
                    if (token) return;
        ...
        function migrateEntryToReplacementContents(windowData, entry, replacement) {
            ...
            entry.sourceContentsRole = 'window-current-contents';
            entry.requiresCopiedTarget = false;
