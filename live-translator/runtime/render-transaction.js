...
(() => {
    ...
    if (typeof defineRuntimeModule !== 'function') {
        throw new Error('[LiveTranslator] runtime module registry is unavailable before runtime/render-transaction.js.');
    }
    ...
    const PHASE_STATUS = Object.freeze({
        ...
        [PHASES.RENDER_COMMITTED]: 'accepted',
    ...
    function createRenderCommit(input = {}) {
        ...
        const commit = Object.assign({
            ...
            noop: status === 'noop',
            terminal: status === 'accepted' || status === 'rejected' || status === 'noop',
            kind,
    ...
    function normalizeCommitStatus(status, phase) {
        ...
        if (value === 'accepted' || value === 'rendered' || value === 'committed') return 'accepted';
        ...
        if (normalizedPhase === PHASES.RENDER_COMMITTED) return 'accepted';
    ...
    defineRuntimeModule('runtime.renderTransaction', {
        PHASES,
        PHASE_STATUS,
        RENDER_COMMIT_KINDS,
