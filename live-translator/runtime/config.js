...
(() => {
    ...
    LiveTranslatorDefine({
        name: 'runtime.config',
        factory(_dependencies, { scope: runtimeScope }) {
            function getScope() {
                return runtimeScope;
            }
            ...
            function validateIntelSettings(settings, logger) {
                const intel = settings && settings.intel;
                if (!intel || typeof intel !== 'object') return;
                validateBooleanSetting(intel.captureWhenGuiClosed, 'intel.captureWhenGuiClosed', logger);

                const limits = intel.limits;
                if (!limits || typeof limits !== 'object') return;
                ['foresightScans', 'foresightMessages', 'archivedItems', 'detachedItems', 'pastJobs'].forEach((key) => {
                    validatePositiveIntegerSetting(limits[key], `intel.limits.${key}`, logger);
                });
            }
            ...
            function validateDiagnosticsSettings(settings, logger) {
                ...
                validateBooleanSetting(diagnostics.enabled, 'diagnostics.enabled', logger);
            }
