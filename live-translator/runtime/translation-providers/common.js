...
(() => {
    ...
    LiveTranslatorDefine({
        name: 'runtime.translationProviders.common',
        factory(_dependencies, { scope: runtimeScope }) {
            ...
            function getGlobalSettings() {
                const settings = runtimeScope && runtimeScope.LiveTranslatorSettings;
                return settings && typeof settings === 'object' ? settings : {};
            }

            function getGlobalTranslatorConfig() {
                const cfg = runtimeScope && runtimeScope.LiveTranslatorConfig;
                return cfg && typeof cfg === 'object' ? cfg : null;
            }

            function getFetch(options = {}) {
                const fetchImpl = options.fetch || options.fetchImpl || runtimeScope.fetch;
                if (typeof fetchImpl !== 'function') {
                    throw new Error('[LiveTranslator] global fetch is unavailable.');
                }
                try {
                    return fetchImpl.bind(runtimeScope);
                } catch (_) {
                    return fetchImpl;
                }
            }
