...
(() => {
    ...
    LiveTranslatorDefine({
        name: 'runtime.performanceProfiler',
        requires: {
            profilerSupport: 'runtime.performanceProfiler.support',
        },
        factory({ profilerSupport }, { scope: globalScope }) {
            const {
                DEFAULT_TARGET_FPS,
            ...
            function createLiveTranslatorPerf(options = {}) {
                ...
                const frameTranslatorDominantMs = (frame) => {
                    const names = [
                        ...
                        'bitmap.pendingDrawUnits.flush.ms',
                        'spriteText.pendingDrawUnits.ms',
                ...
                const summarizeHotFrameSide = (frame) => {
                    ...
                    const pendingDrawUnitsFlush = getFrameTimingGroup(frame, 'translator', 'bitmap.pendingDrawUnits.flush.ms');
                    const pendingDrawUnitsSubscriber = getFrameTimingGroup(frame, 'translator', 'bitmap.pendingDrawUnits.subscriber.ms');
                    const spritePendingDrawUnits = getFrameTimingGroup(frame, 'translator', 'spriteText.pendingDrawUnits.ms');
                    ...
                    return {
                        ...
                        translator: {
                            ...
                            pendingDrawUnitsFlushMs: roundMetric(timingTotal(pendingDrawUnitsFlush)),
                            pendingDrawUnitsFlushCalls: timingCount(pendingDrawUnitsFlush),
                            pendingDrawUnits: getFrameCounter(frame, 'translator', 'bitmap.pendingDrawUnits.flush.units'),
                            pendingDrawUnitsSubscriberMs: roundMetric(timingTotal(pendingDrawUnitsSubscriber)),
                            spritePendingDrawUnitsMs: roundMetric(timingTotal(spritePendingDrawUnits)),
