...
(() => {
    ...
    LiveTranslatorDefine({
        ...
        factory() {
            ...
            function createLoggerBundle(options = {}) {
                const {
            ...
            function createTelemetryChannel(options = {}) {
                ...
                function logTranslation(event, text, result = null, timing = null) {
                    ...
                    switch (event) {
                        ...
                        case 'override':
                            if (logger.shouldLog && logger.shouldLog('debug')) {
                                logger.debug(`[TRANSLATE|${timestamp}] OVERRIDE: "${preview(text)}" → "${preview(result)}"`);
                            }
                            break;
                        case 'cache_miss':
                            if (logger.shouldLog && logger.shouldLog('debug')) {
                ...
                function logDraw(event, text, x, y, extraInfo = {}) {
                    ...
                    switch (event) {
                        case 'original':
                            ...
                            break;
                        case 'redraw':
                            // Stubbed because per-frame redraw telemetry is too noisy at trace volume.
                            ...
                            break;
                        case 'bypass':
                            // Stubbed because bypass telemetry is too noisy at trace volume.
                            // logger.trace(`[DRAW|${timestamp}] BYPASS at (${x},${y}): "${preview(text)}"`);
                            break;
                        case 'queue':
