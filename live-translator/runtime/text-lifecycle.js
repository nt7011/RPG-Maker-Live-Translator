...
(() => {
    ...
    if (typeof defineRuntimeModule !== 'function') {
        throw new Error('[LiveTranslator] runtime module registry is unavailable before runtime/text-lifecycle.js.');
    }
    ...
    const RECORD_EVENT_STATUSES = Object.freeze({
        'item.render_queued': 'completed',
    ...
    function statusFromRecordEvent(eventType, fallback = '') {
        const key = String(eventType || '').trim().toLowerCase();
        if (!key || !Object.prototype.hasOwnProperty.call(RECORD_EVENT_STATUSES, key)) {
            return fallback ? normalizeStatus(fallback, fallback) : '';
        }
        return normalizeStatus(RECORD_EVENT_STATUSES[key], fallback || RECORD_EVENT_STATUSES[key]);
    }
    ...
    defineRuntimeModule('runtime.textLifecycle', {
        STATUS_DEFINITIONS,
        STATUS_ALIASES,
