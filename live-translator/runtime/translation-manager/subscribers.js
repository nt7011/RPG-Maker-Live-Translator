...
(() => {
    ...
    function createController(scope = {}) {
        ...
        function settleSubscriber(subscriber, kind, value) {
            ...
            scope.translationIntel.recordLazy(`subscriber.${subscriber.status}`, () => ({
                jobId: subscriber.job.id,
                subscriberId: subscriber.id,
                recordId: subscriber.recordId || '',
                hook: subscriber.hook || '',
                message: kind === 'resolve' ? '' : scope.translationIntel.formatError(value),
            }));
            if (kind === 'resolve') {
        ...
        function cancelSubscriber(subscriber, reason = 'canceled', options = {}) {
            ...
            scope.translationIntel.increment('canceled');
            ...
            if (!stillActive && cancelOptions.abortJob === true) {
                ...
                if (job.status === 'queued') {
                    ...
                    forgetJobKey(job);
                    job.status = 'canceled';
                    scope.translationIntel.recordLazy('job.canceled', () => ({
                        jobId: job.id,
                        hook: job.hook || '',
                        reason: scope.translationIntel.formatError(error),
                        status: 'queued',
                    }));
                    scope.translationIntel.rememberJob(job, 'canceled', {
                        reason: scope.translationIntel.formatError(error),
                    });
                } else if (job.status === 'running' && job.controller && typeof job.controller.abort === 'function') {
                    try { job.controller.abort(error); } catch (_) {}
                    scope.translationIntel.recordLazy('job.abort_requested', () => ({
                        jobId: job.id,
                        hook: job.hook || '',
                        reason: scope.translationIntel.formatError(error),
                    }));
                ...
                job.detached = true;
                scope.translationIntel.recordLazy('job.detached', () => ({
                    jobId: job.id,
                    hook: job.hook || '',
                    reason: scope.translationIntel.formatError(error),
                    status: job.status || '',
        ...
        function setSubscriberPriority(subscriber, priority, reason = '') {
            ...
            scope.translationIntel.increment('priorityChanges');
            scope.translationIntel.recordLazy('priority.changed', () => ({
                jobId: subscriber.job.id,
                subscriberId: subscriber.id,
                recordId: subscriber.recordId || '',
                hook: subscriber.hook || '',
        ...
        function createSubscriber(job, request) {
            ...
            scope.translationIntel.recordLazy('subscriber.added', () => ({
                jobId: job.id,
                subscriberId: subscriber.id,
                recordId: subscriber.recordId,
                hook: subscriber.hook,
    ...
    LiveTranslatorDefine({
        name: 'runtime.translationManager.subscribers',
        factory() {
