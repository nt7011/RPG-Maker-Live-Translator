...
(() => {
        ...
        async function refreshCapacityIfNeeded(force = false) {
            ...
            scope.capacityPromise = Promise.resolve()
                ...
                .then((capacity) => {
                    ...
                    scope.translationIntel.recordLazy('capacity.refreshed', () => ({
                ...
                .catch((error) => {
                    ...
                    scope.capacityExpiresAt = Date.now() + capacityRefreshMs;
                    scope.lastCapacityRefreshAt = Date.now();
                    scope.lastCapacityRefreshError = scope.translationIntel.formatError(error);
                    scope.translationIntel.recordLazy('capacity.failed', () => ({
                        capacity: scope.providerCapacity,
    ...
    LiveTranslatorDefine({
        name: 'runtime.translationManager.queue',
        factory() {
