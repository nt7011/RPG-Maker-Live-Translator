...
(() => {
    ...
    function createController(scope = {}) {
        ...
        function notifyDelta(job, partial) {
            ...
            scope.translationIntel.increment('streamDeltas');
            if (!job.lastDeltaEventAt || job.lastDeltaAt - job.lastDeltaEventAt >= 1000) {
                job.lastDeltaEventAt = job.lastDeltaAt;
                scope.translationIntel.recordLazy('stream.delta', () => ({
                    jobId: job.id,
                    hook: scope.translationIntel.getJobHook(job),
        ...
        function startJob(job) {
            ...
            scope.translationIntel.increment('dispatched');
            scope.translationIntel.recordLazy('job.dispatched', () => ({
                jobId: job.id,
                hook: scope.translationIntel.getJobHook(job),
                priority: job.effectivePriority,
                stream: job.stream,
                subscribers: scope.translationIntel.getActiveSubscribers(job).length,
            ...
            runProviderWithRetries(job, controller ? controller.signal : undefined)
                .then((translated) => {
                    ...
                    scope.translationIntel.increment('completed');
                    scope.translationIntel.recordLazy('job.completed', () => ({
                        jobId: job.id,
                        hook: scope.translationIntel.getJobHook(job),
                        priority: job.effectivePriority,
                        stream: job.stream,
                    ...
                    scope.translationIntel.rememberJob(job, 'completed');
                    job.subscribers.forEach((subscriber) => {
                ...
                .catch((error) => {
                    ...
                    job.lastError = error;
                    if (job.status === 'failed') scope.translationIntel.increment('failed');
                    scope.translationIntel.recordLazy(`job.${job.status}`, () => ({
                        jobId: job.id,
                        hook: scope.translationIntel.getJobHook(job),
                        priority: job.effectivePriority,
                        stream: job.stream,
                        elapsedMs: Date.now() - (job.startedAt || Date.now()),
                        error: scope.translationIntel.formatError(error),
                        textPreview: preview(job.key, 72),
                    }));
                    scope.translationIntel.rememberJob(job, job.status, {
                        error: scope.translationIntel.formatError(error),
                    });
                    job.subscribers.forEach((subscriber) => {
        ...
        async function runProviderWithRetries(job, signal) {
            ...
            while (true) {
                ...
                try {
                    ...
                    scope.translationIntel.increment('retries');
                    scope.translationIntel.recordLazy('job.retry', () => ({
                        jobId: job.id,
                        hook: scope.translationIntel.getJobHook(job),
                        attempt,
                        retryInMs: waitMs,
    ...
    LiveTranslatorDefine({
        name: 'runtime.translationManager.runner',
        factory() {
